...
! interfaces for the adjoint, trace and determinant operations
	...
	INTERFACE OPERATOR(.DET.)
		MODULE PROCEDURE DET_DCM,DET_MP
	END INTERFACE
CONTAINS
...
! write a matrix into a file passed via argument FPTR
	SUBROUTINE WRITE_DCM(FPTR,MAT,SZ)
		...
		INTEGER*4, DIMENSION(2), INTENT(IN) :: SZ
		INTEGER*4 :: FPTR
		INTEGER*4 :: RINDEX,CINDEX
		DO RINDEX=1,SZ(1)
			DO CINDEX=1,SZ(2)
				WRITE(FPTR,"(a)",ADVANCE="NO")"("
				WRITE(FPTR,"(E8.2)",advance="no")REAL(MAT(RINDEX,CINDEX))
				WRITE(FPTR,"(a)",ADVANCE="NO")","
				WRITE(FPTR,"(E8.2)",advance="no")AIMAG(MAT(RINDEX,CINDEX))
				WRITE(FPTR,"(a)",ADVANCE="NO")")	"
			END DO
			WRITE(FPTR,"(a)",ADVANCE="NO")NEW_LINE('a')
		END DO
	END SUBROUTINE
...
PRINT"(A,I0,A,I0,a)","(",TEST%SZ(1),",",TEST%SZ(2),")"
