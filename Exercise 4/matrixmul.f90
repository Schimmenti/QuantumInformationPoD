...
subroutine write_binary_real4(fileName,fileID,mat,nRows,nCols,rowMajor)
	character(*), INTENT(IN) :: fileName
	integer*4 :: ii,jj,elSz
	integer*4, INTENT(IN) :: nRows,nCols, fileID
	logical, INTENT(IN) :: rowMajor
	real*4, dimension(:,:), INTENT(IN) :: mat
	elSz = 4
	OPEN(unit=fileID,file=fileName, form="unformatted", action="write")
	write(fileID)nRows
	write(fileID)nCols
! number of bytes
	write(fileID)elSz
